...
import flash.net.SharedObject;
   ...
   public class Parameters
   {
      ...
      // RANDOM1 = client->server RC4 key. "B1A5ED" is CORRECT (matches the server's
      // configured key in wServer.json); HELLO decrypts cleanly with it. (Do NOT use the
      // hardcoded _clientKey "811f..." — that's an unused default; it garbles HELLO.)
      ...
      public static function isGpuRender():Boolean {
         // ROTF debug: FORCE the software renderer (no Stage3D/GPU dependency at all).
         // Menus render but the in-game world was black under GPU/Stage3D, which points
         // to no working hardware context (RDP/VM/weak GPU). The software path (map_.graphics)
         // is the author's shipped default and needs no GPU. If this renders the Nexus,
         // the GPU path was the problem. (Revert to !GPURenderError to use GPU later.)
         return false;
      }

...
import kabam.rotmg.ui.view.NotEnoughGoldDialog;
   ...
   public class GameServerConnection
   {
      ...
      {
         // ROTF: null-safe so a missing class/skin selection can't NPE and block the CREATE
         // packet (which left the client stuck on a black load screen). Always sends.
         var charClass:CharacterClass = this.classesModel.getSelected();
         if(charClass == null && this.classesModel.getCount() > 0)
         {
            charClass = this.classesModel.getClassAtIndex(0);
         }
         var create:Create = this.messages.require(CREATE) as Create;
         create.classType = charClass != null ? charClass.id : 0x030e; // 0x030e = Wizard
         var skinId:int = 0;
         if(charClass != null && charClass.skins != null)
         {
            var sk:* = charClass.skins.getSelectedSkin();
            if(sk != null)
            {
               skinId = sk.id;
            }
         }
         create.skinType = skinId;
      ...
      {
         // ROTF debug: server-visible signal that onMapInfo ran (shows as 'Handling packet
         // PONG' in the server log; works with the RELEASE projector, no debugger needed).
         try
         {
            var dbgPong:Pong = this.messages.require(PONG) as Pong;
            dbgPong.serial_ = 31337;
            dbgPong.time_ = getTimer();
            this.serverConnection.sendMessage(dbgPong);
         }
         catch(epd:Error) {}
         trace("[ROTF] onMapInfo ENTER createCharacter=" + this.createCharacter_ + " charId=" + this.charId_);
         try
         {
            this.gs_.applyMapInfo(mapInfo);
            trace("[ROTF] applyMapInfo OK");
         }
         catch(e:Error)
         {
            trace("[ROTF] applyMapInfo FAILED: " + e + "\n" + e.getStackTrace());
         }
         this.rand_ = new Random(mapInfo.fp_);
         trace("[ROTF] onMapInfo about to " + (this.createCharacter_ ? "create()" : "load()"));
         try
         {
            ...
            trace("[ROTF] create/load SENT ok");
         }
         catch(e2:Error)
         {
            trace("[ROTF] create/load FAILED: " + e2 + "\n" + e2.getStackTrace());
         }
      }

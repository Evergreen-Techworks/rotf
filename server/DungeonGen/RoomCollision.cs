...
namespace DungeonGenerator {
	public class RoomCollision {
		...
		bool HitTest(int x, int y, Rect bounds) {
			var key = new RoomKey(x, y);
			// net8 added CollectionExtensions.GetValueOrDefault, which collides with
			// RotMG.Common's DictionaryExtensions overload — qualify to disambiguate.
			var roomList = System.Collections.Generic.CollectionExtensions.GetValueOrDefault(rooms, key, (HashSet<Room>)null);
			if (roomList != null) {

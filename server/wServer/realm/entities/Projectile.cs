...
namespace wServer.realm.entities
{
    ...
    public class Projectile : Entity
    {
        ...
        public Projectile(RealmManager manager, ProjectileDesc desc)
            // Fall back to Blade (0x0006) if the projectile's object id isn't loaded, instead of
            // throwing KeyNotFoundException every tick (imported enemies can reference projectiles
            // whose objects we don't have). Data is also sanitized at import; this is the safety net.
            : base(manager, manager.Resources.GameData.IdToObjectType.TryGetValue(desc.ObjectId, out var ot) ? ot : (ushort)0x0006)
        {
            ProjDesc = desc;
        }

...
namespace wServer.realm
{
    ...
    class Oryx
    {
        ...
        // ROTF Galactic Zones (D3): realm-boss defeat counter -> Mothership finale.
        // Recovered from video: "<Persona> The <Boss> has been defeated! (N/25)" (some tiers use /30).
        private int _galacticKills;
        private const int GalacticGoal = 25;
        private static readonly string[] Personas =
        {
            "Pirate", "Chimera", "Blob", "Drake", "Harpy", "Reaper", "Goblin", "Spider", "Minotaur",
            "Beholder", "Leviathan", "Bat", "Dragon", "Sprite", "Medusa", "Starforce", "Djinn", "Orc"
        };
            ...
            new Dictionary<TerrainType, Tuple<int, Tuple<string, double>[]>>()
        {
            ...
            { TerrainType.Mountains, Tuple.Create(
                100, new []
                {
                    ...
                    // ROTF recovered bosses (rare realm gods until their dungeons are wired)
                    Tuple.Create("Ortar", 0.04),
                    Tuple.Create("Cracked Core", 0.03),
                    Tuple.Create("God of Reptilians", 0.04),
                    // ("Dwarf King" removed from Mountains pool — it's a base MidForest mini-boss, not an OC realm god)
                    Tuple.Create("Twilight Archmage", 0.03),
                    Tuple.Create("Epic Ghost of Skuld", 0.03),
                    Tuple.Create("Stonetaker", 0.03),
                    // backlog batch 2026-06-04 — recovered realm/meme bosses (rare realm gods)
                    Tuple.Create("Marble Colossus", 0.03),
                    Tuple.Create("Lycaon", 0.03),
                    Tuple.Create("Arcanica", 0.03),
                    Tuple.Create("Cookie Monster", 0.03),
                    Tuple.Create("Carrot God", 0.03),
                    Tuple.Create("Mewtwo", 0.03),
                    Tuple.Create("God of Luck", 0.03),
                    Tuple.Create("Giant Oryx Chicken", 0.03),
                    Tuple.Create("The Mysterious Card", 0.03),
                    Tuple.Create("MH Wave Controller", 0.03),
                    Tuple.Create("Creepy Talking Jack-O-Lantern", 0.03),
                    Tuple.Create("Uvuvwuwuwe", 0.03),
                    Tuple.Create("Smiling Dragon", 0.03),
                    Tuple.Create("Warbringer", 0.03),
                    Tuple.Create("Shade of Novus", 0.03),
                    Tuple.Create("Pulse", 0.03),
                    Tuple.Create("Skeletal Monstrosity", 0.03),
                    Tuple.Create("The Puppet Master", 0.03),
                    Tuple.Create("God of Amber Stones", 0.03),
                    Tuple.Create("Queen of Hearts", 0.03),
                    Tuple.Create("Jon Bilgewater the Pirate King", 0.03),
                    Tuple.Create("Kestora the Lava God", 0.03),
                    Tuple.Create("Strange Magician", 0.03),
                    Tuple.Create("Strange Priest", 0.03),
                    Tuple.Create("Mad Jester", 0.03),
                    Tuple.Create("Nikad the Defiler", 0.03),
                    Tuple.Create("Mushroom", 0.03),
                    Tuple.Create("Bedlam - God of Chaos", 0.03),
                    Tuple.Create("The Magical lord of sky", 0.03),
                    Tuple.Create("Time Lord", 0.03),
                    Tuple.Create("Helios", 0.03),
                    Tuple.Create("Ruins of the Mad God Core", 0.03),
                    Tuple.Create("Marble Defender", 0.03),
                    Tuple.Create("Legon the Weather God", 0.03),
                    Tuple.Create("Euryale the Snake Goddess", 0.03),
                    Tuple.Create("Legendary Dragon Kakashi", 0.03),
                    Tuple.Create("Masha, the God of War", 0.03),
                    Tuple.Create("Resthla, The Twisted Mermaid", 0.03),
                    Tuple.Create("Ascended Septavius the Ghost Goddess", 0.03),
                    Tuple.Create("Infused Elven King", 0.03),
                    Tuple.Create("White Dragon Lord", 0.03),
                    Tuple.Create("Suspicious Looking Dragon", 0.03),
                    Tuple.Create("Urios, God of Elements", 0.03),
                    Tuple.Create("The Beekeeper", 0.03),
                    Tuple.Create("Putin", 0.03),
                    Tuple.Create("Wind Elemental", 0.03),  // ROTF realm-event boss (DOCUMENTED-from-chat taunt)
                    Tuple.Create("Wizzard Clone", 0.03),   // ROTF clone-type boss (shared clone-boss taunt, sic name)
                    Tuple.Create("Bloodsworn Knight", 0.03), // NEW ROTF galactic boss (Ruins of the Mad God; holding spot until Phase D3)
                    Tuple.Create("Asylum Guard", 0.03),    // ROTF Asylum-dungeon boss (frame-confirmed squawk taunt)
                    Tuple.Create("Hades", 0.03),           // NEW ROTF soul/energy boss (VERBATIM taunt, VotR 2.0 Beta source)
                    // Nyx of Fire — rare fire/demon WORLD BOSS (evil-player phase + clone)
                    Tuple.Create("Nyx of Fire", 0.02),
                    // meme "Encounter" realm event bosses (verbatim recovered taunts)
                    Tuple.Create("Breathing Encounter", 0.03),
                    Tuple.Create("Smashing Encounter", 0.03),
                    Tuple.Create("Shadow Encounter", 0.03),
                    Tuple.Create("Flying Encounter", 0.03)
                })
            },
        };
        #endregion
        ...
        private void BroadcastPersona(string persona, string message)
        {
            _world.Manager.Chat.Persona(_world, persona, message);
        }

        public void OnPlayerEntered(Player player)
        {
            // ROTF revival, deadpan-branded as the "Ordinary Client" (Elon-style understated name)
            // with a subtle heritage wink (never naming Revenge of the Fallen outright).
            player.SendInfo("Welcome to the Ordinary Client. Just an ordinary client. Nothing unusual here.");
            player.SendInfo("Pay no attention to the fallen. They have a long memory.");
            player.SendEnemy("Oryx the Mad God", "You are food for my minions!");
            player.SendInfo("Use [WASDQE] to move; click to shoot!");
            player.SendInfo("Type \"/help\" for more help. Please report any bugs and exploits.");
        }
        ...
        private void SpawnEvent(string name, ISetPiece setpiece)
        {
            ...
            // ROTF event feed: announce the spawn via a random persona narrator
            // ("<Persona> A <Boss> has just spawned!") — pairs with the defeat counter in OnEnemyKilled.
            if (!Closing)
                BroadcastPersona(Personas[_rand.Next(Personas.Length)], $"A {name} has just spawned!");
        }
        ...
        public void OnEnemyKilled(Enemy enemy, Player killer)
        {
            ...
            // ROTF Galactic Zones (D3): each realm-boss/god defeat fills the galactic counter
            if (!Closing)
            {
                _galacticKills++;
                var persona = Personas[_rand.Next(Personas.Length)];
                BroadcastPersona(persona,
                    $"The {enemy.ObjectDesc.DisplayName} has been defeated! ({_galacticKills}/{GalacticGoal})");

                if (_galacticKills >= GalacticGoal)
                {
                    // Galactic finale. The real Mothership + 4 tier-worlds need NEW sprites/maps
                    // (post-2019 content, absent from our data) -> until those exist, climax with a
                    // gauntlet of the galactic bosses we DO have coded, then close to the Castle.
                    BroadcastMsg("The Mothership descends from the void...");
                    _world.Manager.Chat.Announce($"{_world.Name}: the Mothership has been summoned!");
                    SpawnEvent("Cube God", new CubeGod());
                    SpawnEvent("Pentaract", new Pentaract());
                    SpawnEvent("Lord of the Lost Lands", new LordoftheLostLands());
                    InitCloseRealm();
                }
            }

...
namespace wServer.realm
{
    public class ChatManager : IDisposable
    {
        ...
        // ROTF Galactic Zones (D3): broadcast as a random "announcer persona" (Pirate, Chimera, ...)
        public void Persona(World world, string persona, string text)
        {
            if (string.IsNullOrWhiteSpace(text))
                return;
            ...
            log.Info("[{0}({1})] <{2}> {3}", world.Name, world.Id, persona, text);
        }

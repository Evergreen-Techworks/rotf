...
namespace wServer.realm
{
    class BoostStatManager
    {
        ...
        // ROTF Runes (Phase D2): equipped runes grant passive stat boosts, like equipment.
        private void ApplyRuneBonus()
        {
            if (_player.Runes == null)
                return;
            var items = _player.Manager.Resources.GameData.Items;
            foreach (var objType in _player.Runes.Equipped)
            {
                if (!items.ContainsKey(objType))
                    continue;
                foreach (var b in items[objType].StatsBoost)
                    IncrementBoost((StatsType)b.Key, b.Value);
            }
        }

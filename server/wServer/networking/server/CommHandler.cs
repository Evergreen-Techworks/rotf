...
namespace wServer.networking.server
{
    ...
    public class CommHandler
    {
        ...
        private void StartReceive(SocketAsyncEventArgs e)
        {
            ...
            try
            {
                // Same Linux synchronous-completion handling as the send path.
                if (!e.AcceptSocket.ReceiveAsync(e))
                    ThreadPool.QueueUserWorkItem(_ => ProcessReceive(e.AcceptSocket, e));
            }
        ...
        private void StartSend(SocketAsyncEventArgs e)
        {
            ...
            try
            {
                // .NET on Linux frequently completes SendAsync SYNCHRONOUSLY (returns false);
                // when it does, the Completed event never fires. Without handling that, the
                // send loop stalls at startup and packets (MapInfo, etc.) are enqueued but
                // never transmitted -> client hangs on a black load screen. Process inline.
                if (!e.AcceptSocket.SendAsync(e))
                    ThreadPool.QueueUserWorkItem(_ => ProcessSend(e.AcceptSocket, e));
            }
            catch (Exception exception)
            {
                _client.Disconnect($"[{_client.Account?.Name}:{_client.Account?.AccountId} {_client.IP}] {exception}");
